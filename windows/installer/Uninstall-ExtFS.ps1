...
if ($LASTEXITCODE -eq 0) {
    ...
    if (-not (Wait-ServiceStopped -Name $serviceName)) {
        Write-Warning 'ExtFS 0.9.6 is intentionally resident for this boot; restart Windows to complete driver removal.'
    }
...
if (Test-Path -LiteralPath $certificateSource) {
    try {
        $certificate = [Security.Cryptography.X509Certificates.X509Certificate2]::new($certificateSource)
...
Remove-ItemProperty -LiteralPath 'HKLM:\SOFTWARE\ExtFS' -Name 'TestSigningRequestedBoot' -ErrorAction SilentlyContinue
