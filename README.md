...
The expected packages are `ExtFS-for-Windows-0.9.5-experimental-x64-setup.exe` and `ExtFS-for-Windows-0.9.5-experimental-arm64-setup.exe`. Both are test-signed experimental kernel-driver packages, not production filesystems. The installer detects the native Windows architecture, then independently verifies the bundled driver PE architecture before installation. See `docs/ARM64_TESTING.md` before replacing any existing ARM64 filesystem driver.
