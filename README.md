...
Version 0.9.6 is an installer-only pending-restart hotfix carrying the already-qualified 0.9.3.0 filesystem driver. It fixes the 0.9.5 preflight that treated any Windows pending-restart indicator as fatal: unrelated PendingFileRenameOperations, Windows Update RebootRequired and CBS RebootPending states are now advisory, while ExtFS-specific pending file replacements still require a restart. When ExtFS itself enables TESTSIGNING, the installer records the current boot and requires one actual Windows restart before it proceeds. The reboot marker is stored only for that ExtFS-requested boot transition and is removed after the reboot or successful load. The 0.9.5 native-architecture fix and 0.9.4 filesystem-service ImagePath fix remain in place. The underlying filesystem feature boundary is unchanged from 0.9.3 and continues to use Infiltratr Common 1.9.0 as the kernel-safe shared compiler-annotation dependency.
...
The expected packages are `ExtFS-for-Windows-0.9.6-experimental-x64-setup.exe` and `ExtFS-for-Windows-0.9.6-experimental-arm64-setup.exe`. Both are test-signed experimental kernel-driver packages, not production filesystems. The installer detects the native Windows architecture, independently verifies the bundled driver PE architecture, and only hard-blocks restart states that are specific to ExtFS or to a TESTSIGNING change requested by ExtFS. For installer-only hotfixes, the previously qualified signed driver/catalog/certificate payload is carried forward unchanged and only the installer shell is rebuilt. The GitHub release publishes the x64 installer used for the Windows test path; ARM64 remains available through the qualified package workflow. See `docs/ARM64_TESTING.md` before replacing any existing ARM64 filesystem driver.
